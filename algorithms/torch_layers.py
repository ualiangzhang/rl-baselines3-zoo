...
class Quantize(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Calculate distances
        distances = (
            th.sum(flat_input**2, dim=1, keepdim=True)
            + th.sum(self._embedding.weight**2, dim=1)
            - 2 * th.matmul(flat_input, self._embedding.weight.t())
        )
        ...
        quantized = th.matmul(encodings, self._embedding.weight).view(input_shape)

        # Loss
        e_latent_loss = F.mse_loss(quantized.detach(), inputs)
        q_latent_loss = F.mse_loss(quantized, inputs.detach())
        if self.reg_weight != 0:
            combs = np.array(list(combinations(range(len(self._embedding.weight)), 2)))
            weight_x = self._embedding.weight[combs[:, 0]]
            weight_y = self._embedding.weight[combs[:, 1]]
            reg1_loss = th.exp(-self.Cd * th.sqrt(F.mse_loss(weight_x, weight_y)))
            max_v = th.linalg.vector_norm(
                self._embedding.weight, ord=float("inf"), dim=1
            )
            reg2_loss = th.mean(
                th.maximum(th.square(max_v) - 1, th.zeros(max_v.shape).to(max_v.device))
            )
            reg_loss = self.reg_alpha * reg1_loss + (1 - self.reg_alpha) * reg2_loss
        else:
            reg_loss = 0
        ...
        # perplexity = th.exp(-th.sum(avg_probs * th.log(avg_probs + 1e-8)))
